...
./minifab invoke -p '"initMarble"' -t '{"asset":"'$ASSET'"}'

ASSET=$( echo '{"name":"asset3","color":"blue","size":70,"owner":"tom","price":103}' | base64 | tr -d \\n )
./minifab invoke -p '"initMarble"' -t '{"marble":"'$ASSET'"}'

# transfers marbles 
ASSET_OWNER=$( echo '{"name":"asset2","owner":"jerry"}' | base64 | tr -d \\n )
./minifab invoke -p '"transferMarble"' -t '{"asset_owner":"'$ASSET_OWNER'"}'
